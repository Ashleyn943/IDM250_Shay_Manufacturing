...
<head>
    ...
    <title>Edit Product | Shay Manufacturing</title>
</head>
<body>
    ...
    <div class="dashboard-container">
        <div class="form-card-centered">
            <h1>Edit Product</h1>
            <p class="form-instruction">Update the product details below.</p>
            <?php
                $id = intval($_GET['id']);
                
                $result = mysqli_query($connection, "SELECT * FROM products p 
                                                                    INNER JOIN products_dimensions pd ON p.id = pd.id 
                                                                    INNER JOIN products_types pt ON p.id = pt.id 
                                                                    WHERE p.id = $id");
            ...
            <form action="../library/cms_alt.php?id=<?php echo $id; ?>" method="POST" class="styled-form">
                ...
                <div class="form-footer-actions">
                    ...
                    <button type="submit" name="update_btn" class="btn">Update Product</button>
                </div>
            </form>
        </div>
    </div>
